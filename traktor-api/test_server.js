...
const onChangeNeededImages = () => {
	...
	if (newImages.length > 0) console.log("needed images changed");

	for (const img of newImages) server.publish("cover", img);
...
const server = Bun.serve({
	...
	routes: {
		...
		"/init": {
			POST: async req => {
				...
				if (data.sessionId === sessionId) {
					...
					for (const update in queue) Object.apply(state, update);
					queue = [];
			...
			POST: async req => {
				...
				if (data.sessionId === sessionId) {
					if (!state) {
						...
						if (queue.length > MAX_QUEUE) resetConnection();
					} else {
		...
		"/cover": {
			...
			POST: async req => {
				...
				if (!path) return new Response("'path' query param missing", { status: 400 });
	...
	websocket: {
		open(ws) {
			...
			for (const img of sentImages) ws.send(img);
